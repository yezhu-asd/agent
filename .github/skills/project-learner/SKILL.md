...
description: "Smart Appointment AI Agent（按摩房智能预约系统）项目知识点打卡/复习教练。按知识域选择或推荐知识点，结合源码与本地真实面试题库进行问答、追问、评分、参考答案讲解和进度记录。默认在会话中持续生效，直到用户明确使用退出关键词结束。Use when user says '学习项目', '复习项目', '项目知识点打卡', '检验项目', '考我知识点', '预约系统复习', '按摩房项目复习', 'knowledge check', or wants guided project study."
---
...
Once this skill is activated in a conversation, keep treating the user as being in an ongoing study session.

Rules:
- Do not ask the user to choose a mode again unless they explicitly ask to switch modes.
- Treat follow-up answers, short replies, and corrections as part of the current study flow.
- Continue asking the next question, follow-up, evaluation, or reference answer until the user exits.
- Only stop using this skill when the user clearly says one of these exit signals: 退出学习, 结束学习, 停止打卡, 退出, 结束, exit, quit.
- If the user says to continue, next question, 下一题, or similar, stay in the same skill and advance the study flow.
- If the user message is ambiguous, prefer continuing the current study session instead of switching away from this skill.

When an exit signal is received, briefly summarize the current progress and stop the study loop.
...
5. Ask up to 2 follow-ups based on what the user actually said.
