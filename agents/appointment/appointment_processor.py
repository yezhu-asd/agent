"""
预约处理器
负责把“预约这件事”从信息收集、技师匹配、推荐确认，到最终落库和回复，整条流程串起来
负责协调整个预约流程
"""
"""
# 作用：处理预约相关的核心业务逻辑，包括从用户输入中提取预约信息，查找合适的技师，处理用户对推荐技师的确认或拒绝，
...
appointment_history 里的已收集信息不会立刻清空，下一条相关预约请求再进来时，有机会继续接着处理, 它不是一个真正的“任务挂起队列”，而是状态回切 + 保留上下文；
 ...
 在预约成功流程中，会检查是否需要推荐技师，如果需要则生成推荐消息并等待用户确认，如果不需要则直接处理预约成功；
