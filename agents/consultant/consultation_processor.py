...
class ConsultationProcessor:
    ...
    COMPLEX_SYMPTOM_PROBES = {
        ...
        '嗓子疼': [
            ('nature', '是干痛还是吞咽时痛？'),
            ('duration', '这个情况持续多久了？'),
            ('concomitant', '有没有伴随发烧、咳嗽、咽部异物感等情况？'),
        ],
        '喉咙痛': [
            ('nature', '是干痛还是吞咽时痛？'),
            ('duration', '这个情况持续多久了？'),
        ...
        '牙疼': [
            ('location', '请问是哪颗牙疼？左边还是右边？'),
            ('nature', '是持续性疼痛还是阵发性疼痛？'),
            ('duration', '这个情况持续多久了？'),
            ('concomitant', '有没有伴随牙龈肿痛、张口困难、发热等情况？'),
        ],
        '失眠': [
            ('nature', '是入睡困难、多梦还是早醒？'),
            ('duration', '这个情况持续多久了？'),
            ('concomitant', '有没有伴随焦虑、心慌、白天精神差等情况？'),
        ],
        '皮肤痒': [
            ('location', '请问痒主要在哪个部位？'),
            ('nature', '是持续痒还是阵发性痒？有没有起皮疹？'),
            ('duration', '这个情况持续多久了？'),
            ('concomitant', '有没有伴随红肿、起泡、脱屑等情况？'),
        ],
        '腰疼': [
            ('location', '请问腰疼主要在哪个位置？'),
            ('nature', '是酸胀、刺痛还是牵扯痛？'),
            ('duration', '这个情况持续多久了？'),
            ('concomitant', '有没有伴随腿麻、弯腰困难、发热等情况？'),
        ],
        '背痛': [
            ('location', '请问背痛主要在哪个位置？上背还是下背？'),
            ('nature', '是钝痛、刺痛还是灼痛？'),
            ('duration', '这个情况持续多久了？'),
            ('concomitant', '有没有伴随胸闷、咳嗽、活动受限等情况？'),
        ],
    }
    ...
    SYMPTOM_DEPARTMENT_MAP = [
        (('牙疼', '牙龈肿', '牙齿', '牙痛', '智齿', '口腔', '牙龈'), '口腔科'),
        (('眼睛疼', '眼睛红', '视力', '眼', '近视', '结膜炎', '眼睛'), '眼科'),
        (('耳朵', '耳鸣', '听力', '耳痛', '中耳炎', '鼻塞', '鼻炎', '鼻', '喉咙痛', '嗓子疼', '咽喉', '扁桃体', '咳嗽', '干咳', '咳', '痰', '咳痰', '有痰', '哮喘', '呼吸道'), '耳鼻喉科'),
        (('皮疹', '过敏', '痒', '起疙瘩', '荨麻疹', '湿疹', '皮炎', '痘痘', '皮肤', '红疹'), '皮肤科'),
        (('妇科', '月经', '白带', '痛经', '阴道'), '妇科'),
        (('宝宝', '孩子', '儿童', '小儿', '婴儿'), '儿科'),
        (('失眠', '睡不着', '焦虑', '抑郁', '心理', '情绪', '压力', '多梦', '早醒'), '心理科'),
        (('腰疼', '腰痛', '腰', '背痛', '颈椎', '腰椎', '关节', '肌肉', '骨头', '扭伤', '腿疼', '肩颈'), '骨科'),
        (('肚子疼', '腹痛', '胃', '腹泻', '拉肚子', '恶心', '呕吐', '腹胀', '消化不良', '便秘', '肠胃', '胃痛'), '内科'),
        (('头疼', '头痛', '头晕', '偏头痛', '神经', '中风', '脑', '头胀'), '神经内科'),
        (('胸痛', '胸口疼', '胸闷', '心慌', '心悸', '心脏', '呼吸', '气短'), '内科'),
        (('发烧', '发热', '感冒', '流感', '高烧', '低烧', '体温高'), '内科'),
        (('外伤', '创伤', '骨折', '伤口', '出血', '缝合', '擦伤'), '外科'),
        (('打喷嚏', '流鼻涕', '鼻塞'), '耳鼻喉科'),
    ]
    ...
    async def _do_doctor_consultation(self, original_query: str, user_reply: str, session_id: str) -> AsyncGenerator[str, None]:
        ...
        slots = self._get_slots()
        slots["doctor_sub_state"] = "awaiting_confirmation"
        # 保存主诉症状（而非最近的追问回答），供后续预约按症状推荐科室
        main_symptom = self._extract_mentioned_symptom(original_query)
        slots["pending_symptom_query"] = main_symptom or original_query.split('，')[0]
        self._set_slots(slots)
